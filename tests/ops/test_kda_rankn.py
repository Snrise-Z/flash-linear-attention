...
def test_chunk_kda_rankn_matches_mkda_recurrent(rank: int, dtype: torch.dtype):
    if device == "cpu":
    ...
    torch.manual_seed(0)
    B, T, H, K, V = 2, 31, 3, 64, 48
    scale = K**-0.5

    q = torch.randn(B, T, H, K, device=device, dtype=dtype)
    k = torch.randn(B, T, H, rank, K, device=device, dtype=dtype)
    v = torch.randn(B, T, H, rank, V, device=device, dtype=dtype)
    log_alpha = -F.softplus(torch.randn(B, T, H, K, device=device, dtype=torch.float32))
    beta = torch.sigmoid(torch.randn(B, T, H, rank, device=device, dtype=torch.float32)).to(dtype)
    h0 = torch.randn(B, H, K, V, device=device, dtype=torch.float32)

    q = F.normalize(q, p=2, dim=-1)
    k = F.normalize(k, p=2, dim=-1)

    tri, tri_ht = chunk_kda_rank_n(
        q=q,
        k=k,
        v=v,
        log_alpha=log_alpha,
...
@pytest.mark.parametrize("rank", [1, 2, 3, 4])
def test_chunk_kda_rankn_backward_matches_reference(rank: int):
    if device == "cpu":
        pytest.skip(reason="Triton kernels are not available on CPU.")
    if IS_INTEL_ALCHEMIST:
        pytest.skip(reason="KDA kernels are not supported on alchemist in CI configs.")
    ...
    torch.manual_seed(0)
    B, T, H, K, V = 2, 17, 2, 48, 32
    scale = K**-0.5

    q = F.normalize(torch.randn(B, T, H, K, device=device, dtype=torch.float16), p=2, dim=-1).requires_grad_(True)
    k = (
        F.normalize(torch.randn(B, T, H, rank, K, device=device, dtype=torch.float16), p=2, dim=-1)
        .requires_grad_(True)
    )
    v = torch.randn(B, T, H, rank, V, device=device, dtype=torch.float16).requires_grad_(True)
    log_alpha = (-F.softplus(torch.randn(B, T, H, K, device=device, dtype=torch.float32))).to(torch.float16).requires_grad_(True)
    beta = torch.sigmoid(torch.randn(B, T, H, rank, device=device, dtype=torch.float32)).to(torch.float16).requires_grad_(True)
    ...
    for name, tri_g, ref_g in zip(names, tri_grads, ref_grads, strict=True):
        torch.testing.assert_close(tri_g.to(torch.float32), ref_g.to(torch.float32), rtol=3e-3, atol=3e-3, msg=name)

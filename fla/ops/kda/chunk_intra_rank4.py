...
def chunk_kda_rank4_fwd_kernel_intra_token_parallel(
    k,
    g,
    beta,
    Akkd,
    ...
    Inputs are expanded to pseudo-time with R_MAX=4 (T = 4 * T_tokens):
      - k:    [B, T, H, K]
      - g:    [B, T, H, K]   (chunk-local cumsum, log2)
      - beta: [B, T, H]

    We zero within-token couplings by enforcing: L[i,j]=0 when floor(i/4)==floor(j/4).
    """
...
def chunk_kda_rank4_fwd_kernel_inter_solve_fused_bt64(
    k,
    g,
    beta,
    Akkd,
    ...
    """
    BT=64, BC=32 fused inter + solve kernel for Akk=(I+L)^{-1} in pseudo-time.

    Computes off-diagonal block L10 (rows [32:64], cols [0:32]) and inverts:

      [I+L00,    0]
      [  L10, I+L11]

    Diagonal blocks' strictly-lower entries are read from `Akkd` (fp32).
    """
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        if i_tc1 < T:
            ...
            b_gqn1 = tl.where(m_tc1[:, None], exp2(b_g1 - b_gn1[None, :]), 0.0)
...
@triton.jit(do_not_specialize=["T"])
def chunk_kda_rank4_fwd_kernel_inter_solve_fused_bt128(
    k,
    g,
    beta,
    Akkd,
    ...
    """
    BT=128, BC=32 fused inter + solve kernel for Akk=(I+L)^{-1} in pseudo-time.

    Computes the off-diagonal blocks among 4 subchunks and inverts the 4x4 block unit-lower-triangular system.
    """
    i_t, i_bh = tl.program_id(0), tl.program_id(1)
    i_b, i_h = i_bh // H, i_bh % H
...
@triton.jit(do_not_specialize=["T"])
def chunk_kda_rank4_fwd_kernel_build_L(
    k,
    g,
    beta,
    L,
    ...
    """
    Build the strictly-lower pseudo-time L matrix (within-token couplings masked) for one (chunk, head)
    as 32x32 blocks and write into `L` with shape [B,T,H,BT].

    This is used for BT=256 (i.e. token chunk_size=64 with R_MAX=4 pseudo-time) to avoid
    materializing large BC=BT/4 blocks in registers.
    """
    i_t, i_blk_pair, i_bh = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    i_b, i_h = i_bh // H, i_bh % H

    if IS_VARLEN:
        # chunk_indices stores (sequence_id, chunk_id) for this BT.
    ...
    # Compute scaling reference g_n at the row-block start (per feature dim).
...
@triton.jit(do_not_specialize=["T", "N"])
def chunk_kda_rank4_bwd_kernel_mask_dAkk_within_token(
    dAkk,
    cu_seqlens,
    N,
    T,
    ...
    """
    In-place mask for rank-4 pseudo-time: zero dAkk[i, j] for within-token strict-lower entries.

    Masked set: j in [token_start, i-1] where token_start = floor(i/4)*4.
    ...
    base_col = (i_t % BT) - r
    cols = base_col + tl.arange(0, 3)
    m_col = tl.arange(0, 3) < r

    base = (bos + i_t) * H * BT + (i_hg * BH + o_h) * BT
    offs = base[:, None] + cols[None, :]
    tl.store(dAkk + offs, tl.zeros([BH, 3], dtype=dAkk.dtype.element_ty), mask=m_h[:, None] & m_col[None, :])


def chunk_kda_rank4_fwd_intra_a_inv(
    k: torch.Tensor,
    gk: torch.Tensor,
    beta: torch.Tensor,
    *,
    ...
    """
    Build the pseudo-time (R_MAX=4) intra-chunk inverse matrix A^{-1} = (I + L)^{-1}.

    Shapes:
      k:    [B, T, H, K]    (pseudo-time, T = 4 * T_tokens)
    ...
    if BT not in (64, 128, 256):
        raise ValueError(f"chunk_kda_rank4_fwd_intra_a_inv requires chunk_size in {{64,128,256}}, got {chunk_size}.")
    ...
    if BT in (64, 128):
        ...
        chunk_kda_rank4_fwd_kernel_intra_token_parallel[grid_diag](
            k=k,
            g=gk,
            beta=beta,
            Akkd=Akkd,
        ...
        grid = (NT, B * H)
        if BT == 64:
            chunk_kda_rank4_fwd_kernel_inter_solve_fused_bt64[grid](
                k=k,
                g=gk,
                beta=beta,
                Akkd=Akkd,
            ...
            chunk_kda_rank4_fwd_kernel_inter_solve_fused_bt128[grid](
                k=k,
                g=gk,
                beta=beta,
                Akkd=Akkd,
    ...
    # L: strictly-lower matrix in the same packed [T,BT] layout.
    L = torch.empty((B, T, H, BT), device=k.device, dtype=torch.float32)
    grid_L = (NT, NB * NB, B * H)
    chunk_kda_rank4_fwd_kernel_build_L[grid_L](
        k=k,
        g=gk,
        beta=beta,
        L=L,
...
def chunk_kda_rank4_bwd_mask_dAkk_within_token(
    dAkk: torch.Tensor,
    *,
    cu_seqlens: torch.LongTensor | None = None,
    chunk_size: int = 64,
    ...
    """
    In-place mask for dAkk produced by `prepare_wy_repr_bwd` when using R_MAX=4 pseudo-time.

    Zeroes all within-token strict-lower coupling entries for each sequence.
    """
    B, T, H, BT = dAkk.shape
    if int(chunk_size) != BT or BT not in (64, 128, 256):
        raise ValueError(
            "chunk_kda_rank4_bwd_mask_dAkk_within_token requires chunk_size == dAkk.shape[-1] and "
    ...
    chunk_kda_rank4_bwd_kernel_mask_dAkk_within_token[grid](
        dAkk=dAkk,
        cu_seqlens=cu_seqlens,
        N=N,
        T=T,

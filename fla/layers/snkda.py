...
class SurpriseNormalizedKimiDeltaAttention(nn.Module):
    """
    Surprise-aware Normalized Kimi Delta Attention (S+NKDA).

    Combine:
    - SKDA: data-dependent surprise features -> beta_sk (write intensity) + g_raw (decay raw input)
    - NKDA: scale normalization beta = beta_sk / (||k||^2 + eps) for scale-invariant updates

    Parallelism:
    - beta and g_raw depend only on current-token projections (k, v, hidden_states, position, head_id),
      not on recurrent state S_{t-1}; chunk-parallel KDA kernels remain applicable.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if beta_norm_eps <= 0:
            raise ValueError("beta_norm_eps must be > 0")

        # If kernel l2norm is enabled, ||k|| is ~1, making beta normalization largely redundant.
        if self.use_beta_norm and self.use_qk_l2norm_in_kernel:
            raise ValueError("use_beta_norm=True is incompatible with use_qk_l2norm_in_kernel=True (normalization degenerates).")
        ...
        # Learnable decay parameterization (same as KDA).
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # uncertainty features (aux logits)
        ...
        head_emb = self.surprise_head_embed(head_ids).to(torch.float32)
        ...
        # NKDA normalization: beta' = beta / (||k||^2 + eps)
        if self.use_beta_norm:
            k_norm_sq = k_norm_sq_f.clamp_min(self.beta_norm_eps).to(beta.dtype)
            beta = beta / k_norm_sq

        # g_raw as amplitude * normalized direction(k)

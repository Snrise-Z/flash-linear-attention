...
class FastSlowNormalizedKimiDeltaAttention(nn.Module):
    """
    Fast/Slow Normalized Kimi Delta Attention (FNKDA).

    Fast/slow two-memory KDA with NKDA-style key-norm beta normalization:
        beta' = beta / (||k||^2 + eps)
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_qk_l2norm_in_kernel:
            raise ValueError("use_qk_l2norm_in_kernel=True is incompatible with FNKDA beta normalization.")
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        beta_fast = torch.sigmoid(beta_base + beta_delta)
        beta_slow = torch.sigmoid(beta_base - beta_delta)
        ...
        k_norm_sq = k.to(torch.float32).square().sum(dim=-1).clamp_min(self.beta_norm_eps)
        beta_fast = (beta_fast / k_norm_sq).to(v.dtype)
        beta_slow = (beta_slow / k_norm_sq).to(v.dtype)
        lam = lam.to(v.dtype)

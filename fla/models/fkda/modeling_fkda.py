...
from fla.layers.fkda import FastSlowKimiDeltaAttention
from fla.models.fkda.configuration_fkda import FKDAConfig
from fla.models.utils import Cache
from fla.modules import FusedCrossEntropyLoss, FusedLinearCrossEntropyLoss, RMSNorm
from fla.modules import GatedMLP as FKDAMLP
...
class FKDABlock(nn.Module):
    def __init__(self, config: FKDAConfig, layer_idx: int):
        super().__init__()
...
class FKDAPreTrainedModel(PreTrainedModel):
    config_class = FKDAConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["FKDABlock"]
...
class FKDAModel(FKDAPreTrainedModel):
    def __init__(self, config: FKDAConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([FKDABlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn("`FKDAModel` does not `output_attentions` now, setting it to `False`.", stacklevel=2)
...
class FKDAForCausalLM(FKDAPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: FKDAConfig):
        super().__init__(config)
        self.model = FKDAModel(config)
        self.vocab_size = config.vocab_size

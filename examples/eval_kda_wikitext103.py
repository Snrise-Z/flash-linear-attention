...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate a KDA model on WikiText-103 (loss/perplexity).")

    p.add_argument("--model", type=str, required=True, help="Path to a trained KDA HF checkpoint directory.")
...
def evaluate_ppl(model, dataloader: DataLoader, device: str) -> dict[str, float]:
    ...
    for batch in dataloader:
        ...
        # HF returns mean loss over non-ignored tokens (ignore_index=-100).
        # Here labels never have -100, so tokens = batch_size * seq_len.
        tokens = labels.numel()

#!/usr/bin/env python
"""
Train standard Transformer (with built-in Attention) on WikiText-103 for 20 epochs.
Adapted from train_kda_wikitext103_epochs20.py
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train standard Transformer on WikiText-103 for max_epochs=20"
    )
    ...
    p.add_argument("--output_dir", type=str, default="exp/attention-wt103-e20")
    p.add_argument("--resume_from_checkpoint", type=str, default=None)

    # Architecture - same as KDA for fair comparison
...
def main() -> None:
    ...
    if "test" in dataset:
        ...
        print(f"[attention] test(best-val): loss={test_metrics['loss']:.6f} ppl={test_metrics['perplexity']:.3f}", flush=True)

...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train a small KDA model on WikiText-103 (HF Trainer).")
    ...
    p.add_argument("--output_dir", type=str, default="exp/kda-wikitext103")
...
def _tokenize_batch(examples: dict[str, list[Any]], tokenizer, eos_token_id: int) -> dict[str, list[list[int]]]:
    ...
    # Add EOS as a document separator (important for WikiText line-based samples).
...
def main() -> None:
    ...
    # We handle chunking ourselves, so avoid tokenizer warnings about "model max length".
    ...
    if torch.cuda.is_available():
        ...
        if n_gpu > 1:
            print(
                f"[warn] torch sees {n_gpu} CUDA devices; HF Trainer may use DataParallel, "
                "which can be flaky with Triton autotune. If you hit issues, try: "
                "`CUDA_VISIBLE_DEVICES=0 ...` or `accelerate launch --num_processes 1 ...`.",
                flush=True,
            )
    ...
    if args.preflight_compile and torch.cuda.is_available():
        ...
        # Do not keep the training model in fp16/bf16 when using HF Trainer AMP.
        # Trainer expects fp32 params + autocast; keeping fp16 params will break GradScaler
        # with "Attempting to unscale FP16 gradients."
        ...
        del tmp_model
        # Restore training model to fp32; Trainer will apply autocast based on fp16/bf16 flags.

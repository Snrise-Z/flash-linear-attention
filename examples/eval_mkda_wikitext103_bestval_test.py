...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate MKDA on WikiText-103 using the run's best validation checkpoint (trainer_state.json)."
    )
    ...
    p.add_argument(
        ...
        help="Override MKDA micro-step readout mode at eval time (default: use checkpoint config).",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    out = {"run_dir": args.run_dir, "best_model_checkpoint": ckpt, "micro_readout_mode": args.micro_readout_mode}
    ...
    print(f"[mkda] best_model_checkpoint={ckpt}", flush=True)
    if "validation" in out:
        print(f"[mkda] val:  loss={out['validation']['loss']:.6f} ppl={out['validation']['perplexity']:.3f}", flush=True)
    if "test" in out:
        print(f"[mkda] test: loss={out['test']['loss']:.6f} ppl={out['test']['perplexity']:.3f}", flush=True)
    print(f"[mkda] wrote {out_path}", flush=True)
